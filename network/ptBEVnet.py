...
class ptBEVnet(nn.Module):
    ...
    def forward(self, pt_fea, xyz_ind, cur_dev):
        ...
        # 把index相同的輸入相�?  https://pytorch-scatter.readthedocs.io/en/latest/functions/scatter.html
        ...
        out_data = torch.zeros(out_data_dim, dtype=torch.float32,device=cur_dev)
        out_data[unq[:, 0], unq[:, 1], unq[:, 2], :] = processed_pooled_data  # 给每一格赋�?
        out_data = out_data.permute(0, 3, 1, 2)  # 维度 0,1,2,3 -> 0,3,1,2  �?4, 32, 480, 360
        ...
        res_data[unq[:, 0], unq[:, 1], unq[:, 2], :] = pooled_res_data  # 给每一格赋�?
        res_data = res_data.permute(0, 3, 1, 2)  # 维度 0,1,2,3 -> 0,3,1,2  �?4, 1, 480, 360
        ...
        return net_return_voxel_data, net_return_pt_fea
