    ...
    def __getitem__(self, index):
        ...
        if self.return_test:
            return torch.from_numpy(processed_label).type(torch.LongTensor), \
                   ...
                   labels, \
                   torch.from_numpy(return_fea).type(torch.FloatTensor), index, indexlist
        else:
            return torch.from_numpy(processed_label).type(torch.LongTensor), \
                   torch.from_numpy(grid_ind), \
                   labels, \
                   torch.from_numpy(return_fea).type(torch.FloatTensor), indexlist
...
def collate_fn_BEV(data):
    ...
    indexlist = [d[4] for d in data]
    return label2stack, grid_ind_stack, point_label, xyz, indexlist
